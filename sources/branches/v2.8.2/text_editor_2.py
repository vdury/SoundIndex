...
class TextPanel(wx.Panel):
    ...
    def addAudioSample(self, start, end):
        ...
        #print section
        if section != 0:
            #setattr(self.doc.TEXT.S[section-1].AUDIO, "start", self.SimplifyAudioLimit(start))
            #setattr(self.doc.TEXT.S[section-1].AUDIO, "end", self.SimplifyAudioLimit(end))
            #self.doc.TEXT.S[section-1].xml_append(
            try:
                ...
                print "wooow"
                #self.doc.TEXT.S[section-1].xml_insert_before(self.doc.TEXT.S[section-1].FORM, (u'AUDIO'))
                new_elem = self.doc.xml_element_factory(None, u"AUDIO")
                self.doc.TEXT.S[section-1].xml_append(new_elem)
