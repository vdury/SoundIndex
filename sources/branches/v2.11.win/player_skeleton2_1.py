...
#bitmapDir = os.path.join(dirName, './')
...
class MediaPanel(wx.Panel):
    ...
    def layoutControls(self):
        ...
        self.mainSizer.Add(self.watch, 0, wx.ALL|wx.LEFT, 5)
        self.mainSizer.Add(ScrollSizer, 0, wx.ALL|wx.CENTER, 5)
    ...
    def buildAudioBar(self):
        ...
        #Spacer
        audioBarSizer.AddSpacer(25)

        #Showing Sentence nbr Text
        self.sentence = wx.StaticText(self, wx.ID_ANY, size = (250,40), style = wx.ALIGN_CENTER)        
        audioBarSizer.Add(self.sentence, 0, wx.RIGHT, 0)

        #Showing audio time Text
        self.audioRange = wx.StaticText(self, wx.ID_ANY, size = (250,40), style = wx.ALIGN_CENTER)        
    ...
    def buildBtn(self, btnDict, sizer):
        ...
        img = wx.Bitmap(os.path.join("./", bmp))
    ...
    def showInitialSoundwave(self, path):
        ...
        if fileExtension == '.mp3':
            wx.MessageBox("Unable to Play media : mp3 not supported",
                          "ERROR",
                          wx.ICON_ERROR | wx.OK)
            return
        else:
        ...
        self.fig.set_canvas(self.imageCtrl)
    ...
    def OnScroll(self, event):
        ...
        if self.play >= 0:
            ...
            #self.fig.Update()
            #self.fig.clf()
            #print self.imageCtrl.plot_data.get_ydata()
            #self.fig.clf()
            #self.imageCtrl.plot_data.()
            fig = matplotlib.pyplot.gcf()
            self.ax.clear()
            #fig.clear()
            #line = self.ax.plot(self.signal, '-')
            ...
            #self.ax.axis('off')
            #self.imageCtrl = FigureCanvasWxAgg(self, -1, fig)

            #plt.close()
            #self.fig.clear()
            #self.imageCtrl.figure.clear()
            #fig.clear()

            self.imageCtrl.plot_data = \
                plt.plot(np.linspace(moment,self.zoom+moment, self.zoom*self.fs), self.signal[moment*self.fs:(moment+self.zoom)*self.fs], '-', color='black')[0]
            ...
            #self.imageCtrl.plot_data.set_ydata(np.linspace(0,0,self.zoom*self.fs))
            self.imageCtrl.draw()
            #plt.close()
            #plt.plot()
            #self.imageCtrl.restore_region(self.imageCtrl.copy_from_bbox(self.ax.bbox))
            #self.fig.set_canvas(self.imageCtrl)
            self.audioRangeUpdate()
            self.Refresh()
            self.overlay    = wx.Overlay()
    ...
    def onPlay(self, event):
        ...
        if not self.mediaPlayer.Play():
            ...
            endset = max(self.x0, self.x1)
            self.endmark = max(self.x0, self.x1)
            #If the marker is set somewhere, see if 
            #we play back in the middle of the marker (where pause has been pressed)
            #Or at the beginning of the marker
            if (self.mediaPlayer.Tell() < offset*1000) | (self.mediaPlayer.Tell() > endset*1000):
                self.mediaPlayer.Seek(offset*1000)
            else:
                self.mediaPlayer.Seek(self.mediaPlayer.Tell())
        elif self.edit == 0:
            self.mediaPlayer.Seek(self.mediaPlayer.Tell())
        else:
            self.mediaPlayer.SetInitialSize()
    ...
    def onTimer(self, event):
        ...
        if self.play == -1:
            ...
            if self.mediaPlayer.Tell() == self.mediaPlayer.Length():
