...
class MediaPanel(wx.Panel):
    ...
    def OnRelease(self, event):
        if self.pressed == 1 & self.edit == 1:
            ...
            if x-self.x0 > 0:
                dc.DrawRectangle(self.x0, 0, x-self.x0, self.imageMaxHeight - 40)
                if self.play != -1:
                ...
                dc.DrawRectangle(x, 0, self.x0-x, self.imageMaxHeight - 40)
                if self.play != -1:
    ...
    def buildAudioBar(self):
        ...
        self.sentence = wx.StaticText(self, wx.ID_ANY, size = (500,40), style = wx.ALIGN_CENTER)        
        audioBarSizer.Add(self.sentence, 0, wx.RIGHT, 0)

        #Showing audio time Text
        self.audioRange = wx.StaticText(self, wx.ID_ANY, size = (500,40), style = wx.ALIGN_CENTER)        
    ...
    def audioRangeUpdate(self):
        ...
        self.audioRange.SetLabel("[%02d:%02d - %02d:%02d]" %(momentmin, moment, endmin, end))
    ...
    def onPlay(self, event):
        ...
        if not self.mediaPlayer.Play():
            ...
            self.playPauseBtn.SetLabel(u"\u25AE" + u"\u25AE")
            self.mediaPlayer.SetInitialSize()
            #self.GetSizer().Layout()
            self.drawCursor()
